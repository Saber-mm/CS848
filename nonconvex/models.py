...
class MLP(nn.Module):
    def __init__(self, dim_in, dim_hidden, dim_out):
        ...
        self.layer_input = nn.Linear(dim_in, 10)
        # self.relu = nn.ReLU()
...
class CNNCifar(nn.Module):
    ...
    def forward(self, x):
        x = self.lrn1(self.pool(F.relu(self.conv1(x))))
        x = self.pool(self.lrn2(F.relu(self.conv2(x))))
        x = x.view(-1, 64 * 5 * 5)
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        x = self.fc3(x)
...
class CNNCifarSmall(nn.Module):
    def __init__(self, args):
        super(CNNCifarSmall, self).__init__()
        self.conv1 = nn.Conv2d(3, 64, 5)
        self.pool = nn.MaxPool2d(2, 2)
        self.lrn1 = nn.LocalResponseNorm(2)
        self.conv2 = nn.Conv2d(64, 64, 5)
